...
`x402` resurrects HTTP status 402 (“Payment Required”) as a real protocol: users sign an authorization payload, the server verifies it off-chain, and content unlocks instantly. Settlement (actual token transfer) happens via facilitator (Coinbase CDP) without forcing readers to wait for an on-chain confirmation.
...
1. Frontend builds `X-PAYMENT` header via `x402PaymentService`.  
...
3. Optional `settlementService` batches author payouts when network is congested. 
...
| Wallets           | AppKit/WalletConnect for EVM & SVM. We never custody funds; users sign in their own wallets.   |
| Secrets           | `.env` per service, injected via deployment pipeline; no secrets in Git.                                     |

Additional notes:

- **API signing**: API-to-API calls (e.g., admin scripts) will include HMAC headers to prevent spoofing.  
...
1. **Devnet x402 harness**: `/x402-test` page exercises requirement fetch, signature generation, header validation, and article unlock for both all supported networks.  
2. **Manual wallet QA**:
