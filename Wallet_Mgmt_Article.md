...
1. **One primary wallet per profile** – this is the address you first published with. All lifetime stats and author metadata live under this canonical wallet/UUID.
...
1. In the dashboard, click the **Manage payout methods** button.  
2. Choose **Remove** or enter a new address and click **Save**.  
...
This safeguards your account so an obsolete wallet can’t keep editing your profile.
...
- You **do not** need to sign in with the new wallet you’re adding. Just paste the address and confirm—it will receive payouts immediately.
- After linking, you can connect that secondary wallet (Phantom, AppKit, etc.) at any time to access the same dashboard, stats, and drafts. We treat primary and secondary equally once they’re on file.
...
- If you already published with two different wallets by accident, contact support—we’ll merge the profiles when the admin tooling ships.  
  - If you hit an “address already linked” error, use the CTA in the modal to reach out to support@gopenny.io for a manual merge.
...
- We never custody funds or private keys; wallet signatures happen entirely client-side via AppKit/WalletConnect and Phantom.  
- Once stored, payouts route automatically—no additional logins needed for the receiving wallet.

---

Questions or issues? Reach out via support@gopenny.io.
